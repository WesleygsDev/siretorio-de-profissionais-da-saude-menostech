...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (!empty($_POST['cropped_image'] ?? '')) {
        ...
        if ($saved) {
            if ($profissional['foto'] && file_exists($uploadDir . $profissional['foto'])) {
                @unlink($uploadDir . $profissional['foto']);
            }
            $foto = $saved;
        }
    }
    ...
    $query = "UPDATE profissionais SET " . implode(", ", $sets) . " WHERE id = $id";

    if (mysqli_query($conn, $query)) {
        $success = 'Profissional atualizado com sucesso!';
        $profissional = mysqli_fetch_assoc(mysqli_query($conn, "SELECT * FROM profissionais WHERE id = $id"));
    } else {
...
<head>
    ...
    <title>Editar Profissional - MenoTech</title>
        ...
        <form method="POST" enctype="multipart/form-data">
            ...
            <div class="row">
                <div class="col-md-6 mb-3">
                    ...
                    <input type="text" name="nome" class="form-control" value="<?php echo $profissional['nome']; ?>" required>
                </div>
                <div class="col-md-6 mb-3">
                    <label class="form-label">E-mail*</label>
                    <input type="email" name="email" class="form-control" value="<?php echo $profissional['email']; ?>" required>
                </div>
            </div>
            <div class="row">
                <div class="col-md-6 mb-3">
                    ...
                    <select name="especialidade" class="form-select" required>
                        ...
                        <?php foreach ($especialidades as $esp): ?>
                            <option value="<?php echo $esp; ?>" <?php echo $profissional['especialidade'] == $esp ? 'selected' : ''; ?>><?php echo $esp; ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                <div class="col-md-3 mb-3">
                    <label class="form-label">Cidade*</label>
                    <input type="text" name="cidade" class="form-control" value="<?php echo $profissional['cidade']; ?>" required>
                </div>
                <div class="col-md-3 mb-3">
                    ...
                    <select name="estado" class="form-select" required>
                        ...
                        <?php foreach ($estados as $est): ?>
                            <option value="<?php echo $est; ?>" <?php echo $profissional['estado'] == $est ? 'selected' : ''; ?>><?php echo $est; ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
            </div>
            <div class="row">
                <div class="col-md-4 mb-3">
                    ...
                    <input type="text" name="instagram" class="form-control" value="<?php echo $profissional['instagram']; ?>" placeholder="@usuario">
                </div>
                <div class="col-md-4 mb-3">
                    <label class="form-label">Telefone</label>
                    <input type="text" name="telefone" class="form-control" value="<?php echo $profissional['telefone']; ?>">
                </div>
                <div class="col-md-4 mb-3">
                    <label class="form-label">Site</label>
                    <input type="url" name="site" class="form-control" value="<?php echo $profissional['site']; ?>">
                </div>
            </div>
            <div class="row">
                <div class="col-md-4 mb-3">
                    <label class="form-label">WhatsApp</label>
                    <input type="text" name="whatsapp" class="form-control" value="<?php echo $profissional['whatsapp'] ?? ''; ?>" placeholder="(DDD) 9xxxx-xxxx">
                </div>
                <div class="col-md-4 mb-3">
                    <label class="form-label">Conselho</label>
                    <select name="conselho_tipo" class="form-select">
                        <option value="">Selecione</option>
                        <?php $ct = $profissional['conselho_tipo'] ?? ''; ?>
                        <option value="CRM" <?php echo $ct === 'CRM' ? 'selected' : ''; ?>>CRM</option>
                        <option value="CRP" <?php echo $ct === 'CRP' ? 'selected' : ''; ?>>CRP</option>
                        <option value="CRN" <?php echo $ct === 'CRN' ? 'selected' : ''; ?>>CRN</option>
                        <option value="CREFITO" <?php echo $ct === 'CREFITO' ? 'selected' : ''; ?>>CREFITO</option>
                        <option value="CREF" <?php echo $ct === 'CREF' ? 'selected' : ''; ?>>CREF</option>
                        <option value="COREN" <?php echo $ct === 'COREN' ? 'selected' : ''; ?>>COREN</option>
                    </select>
                </div>
                <div class="col-md-4 mb-3">
                    <label class="form-label">Número do registro</label>
                    <input type="text" name="conselho_numero" class="form-control" value="<?php echo $profissional['conselho_numero'] ?? ''; ?>">
                </div>
            </div>
            <div class="row">
                <div class="col-md-4 mb-3">
                    <label class="form-label">RQE (médicos)</label>
                    <input type="text" name="rqe" class="form-control" value="<?php echo $profissional['rqe'] ?? ''; ?>">
                </div>
            </div>
            <div class="row">
                <div class="col-md-4 mb-3">
                    <label class="form-label">Atendimento</label>
                    <?php $att = $profissional['atendimento'] ?? ''; ?>
                    <select name="atendimento" class="form-select">
                        <option value="">Selecione</option>
                        <option value="online" <?php echo $att === 'online' ? 'selected' : ''; ?>>Online</option>
                        <option value="presencial" <?php echo $att === 'presencial' ? 'selected' : ''; ?>>Presencial</option>
                        <option value="ambos" <?php echo $att === 'ambos' ? 'selected' : ''; ?>>Online e Presencial</option>
                    </select>
                </div>
                <div class="col-md-4 mb-3">
                    <label class="form-label">CEP</label>
                    <input type="text" name="cep" class="form-control" value="<?php echo $profissional['cep'] ?? ''; ?>">
                </div>
                <div class="col-md-4 mb-3">
                    <label class="form-label">Bairro</label>
                    <input type="text" name="bairro" class="form-control" value="<?php echo $profissional['bairro'] ?? ''; ?>">
                </div>
            </div>
            <div class="mb-3">
                <label class="form-label">Endereço</label>
                <input type="text" name="endereco" class="form-control" value="<?php echo $profissional['endereco'] ?? ''; ?>" placeholder="Rua, número, complemento">
            </div>
            <div class="mb-3">
                <label class="form-label">Biografia</label>
                <textarea name="biografia" class="form-control" rows="4"><?php echo $profissional['biografia']; ?></textarea>
            </div>
            <div class="mb-3">
                <label class="form-label">Foto</label>
                <?php if ($profissional['foto']): ?>
                    <img src="../uploads/<?php echo $profissional['foto']; ?>" alt="Foto" width="140" height="90" class="mb-2 d-block" style="object-fit:cover; border-radius:12px;">
                <?php endif; ?>
